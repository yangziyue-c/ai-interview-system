...
> 目录结构保持交付包原样（`代码/`、`数据/`、`vector_db/`、`说明/` 同级），
> 使脚本内的相对路径定位（`_PKG_ROOT` = 脚本所在目录的父目录）自动生效，无需配置任何路径。
>
> **向量库目录名必须是纯 ASCII（`vector_db/`；旧名「向量库」已废弃）**：
...
4. **两段式检索 + 双模式**：bge-m3 快速召回 → reranker 精排 → 按原题ID去重 → Top-N。
   ...
   - **深挖模式**（`--expand`）：命中 1 题后按原题ID 取该题全部层级片段（原题/L1/L2/L3/得分点/铺垫/变体），支撑追问、逐点评分、降级引导；只取 1 条会丢信息。
...
4. **断点续跑安全**：`build_v2_done.json` 记录已完成文件，但严禁同时运行多个构建进程（col.add 对重复 id 会崩）。
...
> 依赖刻意不写入 `backend/requirements.txt`：主流程（8001 主后端 / 8002 评估服务）
> 不依赖它们，写进去会让每个新环境都多下载 2.5GB。
> `backend/start.py` 会检测并在缺失时按需安装，安装失败则跳过 RAG 服务、不影响主流程。

## 1 号适配说明（2026-09-14）

本副本相对 P5 原交付包做了以下改动。P5 后续更新交付包时，请保留这些改动，
...
| 4 | 五个脚本 | `_pick()` 全部未命中时 `raise FileNotFoundError` | 原实现返回无效路径 → chromadb 静默创建空库，表现为「检索始终无结果」而非「路径配错」 |
| 5 | `02/03/05` | `torch.set_num_threads(8)` → 默认取 CPU 核心数 | 28 核机器上写死 8 会浪费算力 |
| 6 | `05` | 启动打印实际命中的向量库/主库路径 + collection 条数 | 路径问题一眼可见 |
...
| 9 | `02/03/04/05` | 向量库目录 `向量库/` → `vector_db/`（脚本自动改名升级）+ 打开前非 ASCII 路径守卫 | chromadb 打不开含非 ASCII 字符的绝对路径（实测 10/10 报 `Error loading hnsw index`，极易误判为索引损坏）；构建侧同样受灾：中文绝对路径建出的库缺 HNSW 索引文件 |
