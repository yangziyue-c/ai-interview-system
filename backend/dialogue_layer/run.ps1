...
# 1) 模型缓存：走本地缓存 + 离线模式
# ------------------------------------------------------------
# 交接说明第七节：D:\A11-Data\hf_cache 是模型缓存，不要动。
# 大小：2026-09-25 实测 9.51 GB / 90 个文件（**只会变大**：每加一个模型就多一份，
# 加了情感模型那波从 8.71 GB 涨上来的）。这句话里的数字**过一段时间就会旧**，
# 别拿它当判据 —— 要核就现量：Get-ChildItem $env:HF_HOME -Recurse -File | Measure-Object Length -Sum
