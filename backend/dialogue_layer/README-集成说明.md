...
这一层是 P2 交付的独立服务，在本项目里作为**可选面试引擎**使用。启用后，出题、追问、五维评分全部委托给它，主后端只负责把每轮问答与最终报告镜像落回本项目的数据库。

## 来源与版本

| 项 | 值 |
| :--- | :--- |
| 来源 | P2 交付包 `A11-AI对话层-后端-1号`（2026-09-23 第二版，`app/__version__ = 1.0.0`） |
...
`start.py` 的 `build_dialogue_env()` 负责注入，原包 `run.ps1` 里的那份默认值指向 2 号的机器，不适用本仓库。
