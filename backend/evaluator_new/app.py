...
- 手动：cd backend && python evaluator_new/app.py

与 backend/evaluator/（3 号原版）的唯一差异
============================================================
**按题评分增强**：`qa_list` 的每一项可携带可选的 `materials` 字段
（由主后端按题干从 questions 表取得：基础得分点 / 进阶得分点 / 单题校准锚点），
本服务将其作为「本题评分参考」拼进对话文本，让 LLM 按「这道题该考什么」评分，
而不是仅凭对话泛泛打分。

设计约束（刻意如此）：
- **依赖倒置**：素材由调用方提供，本服务不查库、保持无状态（与 3 号原版一致）；
- **向后兼容**：不传 `materials` 时输出与 3 号原版**逐字一致**；
- **Prompt 模板零改动**（evaluation_prompts.py 未改）——素材作为对话文本的一部分注入。

素材来源：V5 知识库（backend/rag/数据/*-v5.json，5012 题）的
`calibration_anchor` / `basic_score_points` / `advanced_score_points` 三个字段。
详见 docs/reports/REPORT_TO_P3_EVALUATOR_NEW.md。
"""
import os
import sys
...
def evaluate():
    ...
    try:
        ...
        # 4. 调用大模型生成评估报告（失败时按 qa_list 篇幅生成降级报告）
        report = call_llm_for_evaluation(prompt, qa_list, position)
...
def build_dialogue_text(qa_list):
    """将qa_list拼接成对话文本（带轮次标记 + 可选题目素材）

    materials（可选，由主后端按题干从题库取）：
        basic_score_points / advanced_score_points —— 本题的得分点
        calibration_anchor —— 单题校准锚点（[技术水平]/[岗位匹配度] 判分标准）

    注入后 LLM 可按「这道题该考什么」评分；**不传 materials 时输出与
    3 号原版逐字一致**（向后兼容，见模块 docstring）。
    """
    dialogue = ""
    for qa in qa_list:
        ...
        materials = qa.get("materials") or {}
        refs = []
        if materials.get("basic_score_points"):
            refs.append(f"基础得分点：{materials['basic_score_points']}")
        if materials.get("advanced_score_points"):
            refs.append(f"进阶得分点：{materials['advanced_score_points']}")
        if materials.get("calibration_anchor"):
            refs.append(f"判分标准：{materials['calibration_anchor']}")
        if refs:
...
def call_llm_for_evaluation(prompt, qa_list=None, position=""):
    """调用DeepSeek API生成评估报告

    qa_list / position 仅用于**失败时**生成按篇幅分档的降级报告——原实现在
    超时/异常/非 200 分支调用无参的 `get_default_report()`，会按「空答卷」算出
    最低档（≈60 分），与 `evaluate()` 异常分支（带参、按篇幅分档）口径不一致：
    答得很好的考生在 LLM 抽风时也会拿到 60 分的「回答偏简略」报告。
    """
    try:
        ...
        if response.status_code != 200:
            ...
            return get_default_report(qa_list, position)
