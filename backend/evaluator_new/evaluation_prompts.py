...
# 通用兜底文案：在册但无专属 Prompt 配置的岗位用之（与 GENERIC_POSITION 共用一份，避免两处漂移）
_GENERIC_KEY_POINTS = (
    "暂无专属考察点配置，请按岗位通用能力（基础功底、问题分析、实践经验、沟通表达）综合评估"
)
_GENERIC_POSITION_DESC = (
    "该岗位暂无专属描述，请按候选人回答的专业性、条理性和与问题本身的契合度进行客观评估。"
)

POSITION_CONFIG = {
    code: {
        **config,
        # ⚠️ 用 `or _GENERIC_*` 而非 `.get(code, "")`：原实现对本文件没有专属文案、
        # 但在 evaluation_weights.POSITION_CONFIG 里已注册的岗位（V5 新增的
        # algorithm / system_design）会返回空串，导致 Prompt 出现「核心考察点：」后
        # 什么都没有——岗位在册却被当成"无配置"。空串与"未注册"应区分开。
        "key_points": _KEY_POINTS.get(code) or _GENERIC_KEY_POINTS,
        "position_desc": _POSITION_DESCS.get(code) or _GENERIC_POSITION_DESC,
    }
...
GENERIC_POSITION = {
    **_SHARED_GENERIC,
    "key_points": _GENERIC_KEY_POINTS,
    "position_desc": _GENERIC_POSITION_DESC,
}
