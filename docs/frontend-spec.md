...
> ⚠️ **接口部分已过期（2026-09-06）**：本文档为前端开发初稿，其中接口规格与
> 后端实际实现大量不符（已由 [FRONTEND_HANDOFF.md](FRONTEND_HANDOFF.md) 勘误）。
> **页面设计部分（本文件主体）仍为有效需求参考；所有接口一律以
> [API.md](API.md) 为唯一权威**，勿按本文件接口部分开发。
