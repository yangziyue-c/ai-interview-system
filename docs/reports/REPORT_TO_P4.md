...
## 7. 关于 P1 的演示前端 frontend_test/（2026-09-07 补充，重要）

仓库根目录另有 **`frontend_test/`** —— 这是 P1 在你们正式前端交付前搭的**零依赖演示前端**
（纯 HTML/CSS/JS，功能对齐本文档页面需求，供项目整体演示），**与你的交付互不相干**：

- `backend/start.bat` 会把它自动拉起到 **5273 端口**并打印访问指引；5273 特意**避开
  5173**（你的 Vite dev 端口），双方同时开发互不冲突；
- 你交付正式前端时**只需按原约定把构建产物放入 `backend/static/`**，`frontend_test/`
  与 start.py 的 5273 拉起会在正式前端就绪后由 P1 停用/删除，你无需处理它；
- 该演示前端已实测通过全流程（注册 → 7 轮面试 → 报告/成长曲线），可作为你页面效果的
  参考实现与联调基线（接口调用方式见第 5 节提示词）。
