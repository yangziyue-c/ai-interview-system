...
> | 你可以 | 说明 |
> |---|---|
> | **直接跑** | 有单元测试（`tests/test_question_bank.py`）+ 效果回归（`scripts/simulate_interview.py`），可先当基线 |
> | **照它改** | 第 4 节按「改动成本从低到高」列了 5 个常见改法，每条写明**改哪里 + 测试怎么写** |
> | **整个推翻** | 只要 `pick_next` 的签名不变，集成层（`app/adapters/ai_interviewer.py`）一行都不用动 |
>
...
## 六、可选的简历素材（2026-09-23 新增，**一期未接入**）

> **一期你不需要改任何代码。** 这一节只是把「简历文本存在哪、怎么取、要注意什么」
> 写清楚，供你判断要不要在下一期做「结合简历提问」。

### 6.1 素材在哪

后端新增了简历导入功能（`POST /api/v1/resumes`）：候选人上传简历（PDF / 图片），
PDF 会被提取成纯文本存进新增的 `resumes` 表。你要用时有两个途径：

| 途径 | 说明 |
| :--- | :--- |
| 读接口 | `GET /api/v1/resumes/latest` → `data.text`（当前用户最近一份简历全文，上限 10000 字） |
| 直接查表 | `resumes` 表：`user_id` / `text_content`（全文）/ `parse_status` / `created_at`，取最新一条即 `ORDER BY id DESC LIMIT 1` |

**务必先看 `parse_status`**：只有 `parsed` 才有 `text_content`；
`image_pending`（图片）、`no_text_layer`（扫描件 PDF）、`garbled`、`failed` 四种情况下它是 `null`。
**不能假设一定拿得到简历**——出题逻辑必须能在没有简历时照常工作（绝大多数用户不会传简历）。

### 6.2 若要做「结合简历提问」，两条路子

| 做法 | 改动范围 | 代价 |
| :--- | :--- | :--- |
| **在 `pick_next` 内部读简历、把片段拼进追问文本** | 只动 `interviewer_new/`，集成层不用改 | 题库抽出的题是固定文本，拼出来的题会脱离题库——**也就脱离了单题校准锚点**，P3 的按题评分素材会跟着失效 |
| **集成层透传 `resume_text` 参数** | 要改 `app/adapters/ai_interviewer.py`（1 号维护） | 只在题库未命中、走到 LLM 兜底时才生效；V5 题库覆盖良好，实际很少触发 |

> 若选第二条，请先说一声，我们商定一个参数名再改，免得两边各加一个。

### 6.3 脱敏（真要做，必须先处理）

简历里含**手机号、身份证号、邮箱、住址**。目前整条链路都在本地、不出网，没有问题；
但**一旦把简历文本发给外部大模型（含 `LLM_API_KEY` 那条兜底路径），就等于把候选人的
个人信息交给了第三方**。外发前请先用正则剔除：

- 手机号 `1[3-9]\d{9}`
- 18 位身份证 `\d{17}[\dXx]`
- 邮箱 `[\w.+-]+@[\w-]+\.[\w.]+`

答辩时评委很可能问「简历里的隐私怎么处理」，这条得有答案。
