# 内网穿透部署说明（演示用）

目标：让评委/观众在 **任何网络**（手机流量、校园网外）通过 **公网地址** 访问你笔记本上运行的后端。

本项目的后端已做好配合：

- 启动时绑定 `0.0.0.0`（监听所有网卡）
- CORS 已配置为通配符 `*`（任何来源的前端页面都可调用 API）
- 前后端同端口（前端 dist 由后端挂载），穿透一个端口即可访问全部功能

> 前置条件：后端已通过 `start.bat` 启动在 **8000 端口**。
> 验证本机连通性：浏览器打开 `http://localhost:8000/api/v1/health`。

---

## 方案一：Sakura Frp（国内速度快，推荐）

1. **注册账号**：打开 https://www.natfrp.com ，注册并完成实名认证（学生认证免费额度够用）。
2. **下载客户端**：官网下载 SakuraFrpLauncher.exe（Windows 启动器），解压后双击运行，用账号登录。
3. **创建隧道**：
   - 左侧点击「隧道」→「创建隧道」
   - 隧道类型：`TCP`（或 `Web`）
   - 服务器节点：选一个**国内节点**（如浙江、四川等，节点列表有延迟数据，选绿色快的）
   - 本地地址：`127.0.0.1`
   - 本地端口：`8000`
   - 名称随意，如 `ai-interview-demo`
4. **启动隧道**：在启动器中点击隧道右侧的开关，等状态变为「运行中」。
5. **获取公网地址**：隧道详情中会显示分配的访问地址，形如：
   - `https://xxx.natfrp.cloud`（Web 类型）
   - 或 TCP 类型映射的公网 `IP:端口`
6. **演示**：把该地址发到手机/微信群，打开即可看到前端页面；接口访问路径不变（`/api/v1/...`）。

> 注意：免费版隧道地址可能**定期更换**，演示当天提前 20 分钟启动隧道并确认地址。

---

## 方案二：NatApp（免费流量，简单）

1. **注册**：打开 https://natapp.cn ，注册账号。
2. **实名认证**：个人中心完成实名认证（免费隧道的必要条件）。
3. **购买免费隧道**：隧道市场 → 购买「免费隧道」（每日可领流量）。
4. **配置隧道**：
   - 隧道类型：`TCP`
   - 本地端口：`8000`
   - 协议：http
5. **下载客户端**：https://natapp.cn/#download 下载 Windows 客户端 natapp.exe。
6. **获取 authtoken**：个人中心 → 我的隧道 → 复制 `authtoken`。
7. **启动隧道**：在命令行执行（把 token 换成你的）：
   ```bat
   natapp.exe -authtoken=你的authtoken
   ```
8. **获取公网地址**：启动后终端会打印：
   ```
   Tunnel Status   online
   Forwarding      http://xxxxxx.natappfree.cc -> 127.0.0.1:8000
   ```
   其中 `http://xxxxxx.natappfree.cc` 就是公网访问地址。
9. **演示**：手机访问该地址即可。

> 注意：免费版 NatApp 首次访问会先显示 NatApp 的提示页，点击「继续访问」进入。

---

## 常见问题

| 问题 | 处理 |
| :--- | :--- |
| 公网打不开，本机正常 | 确认隧道状态为「运行中/online」，且本地端口填的是 8000 |
| 页面打开但接口 404/报错 | 接口路径为 `/api/v1/...`，确认拼写；公网地址后不用再加端口 |
| 提示 CORS 错误 | 本项目已配置 `*` 通配，若仍报错检查是否访问了旧地址 |
| 免费隧道限速/限流量 | 演示前清空浏览器缓存，语音文件控制在 5MB 内；可切换 Sakura Frp 备用节点 |
| 校园网封隧道端口 | 换 Sakura Frp 的 Web 类型隧道（走 80/443 端口，一般可通） |

## 局域网演示（同一 WiFi，无需穿透）

同学/评委与笔记本连**同一 WiFi** 时，直接访问：

```
http://<你电脑的局域网IP>:8000
```

查看本机 IP：`ipconfig` 中「无线局域网适配器 WLAN」的 IPv4 地址（如 192.168.x.x）。
