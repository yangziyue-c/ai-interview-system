...
> **2026-09-06 更新**：面试官算法已原生落地主项目（`backend/interviewer/`，
> 题库策略为最高优先级数据源）。本教程第 6 节的外部服务接入仅作**扩展位**——
> 题库策略未命中（如新岗位无题）时才会调用 `AI_INTERVIEWER_URL`。开发库三岗位
> 451 题常驻命中，联调外部服务请先用无题岗位（或清空该岗位题目）触发。

1. ✅ 启动时全量拉取成功：三岗位共 451 题（backend 151 + frontend 150 + test_engineer 150）
2. ✅ `POST /generate`（round=1）返回开场热身 + easy 题
3. ✅ 追问触发：候选人回答含 L1 关键词 → 返回对应追问；连续追问依次走 L2 → L3 → 换新题
4. ✅ round=6/7 返回收尾交流题（最后两轮强制收尾）
5. ✅ 主后端完整面试 7 轮走通并自动出报告（出题由题库策略驱动，无需外部服务）
6. ✅ 主后端回归：`cd backend && pytest`（58 个用例全绿）
